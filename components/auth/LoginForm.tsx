   ...
   const [state, action, isPending] = useActionState(login, loginInitialState)    
    return (
        <div className="flex w-full max-w-full flex-col gap-2">
                ...
                <button type="submit" disabled={isPending} className="hover:shadow-md w-full rounded-md border-1 border-gray-300 bg-lime-300 px-4 py-2 text-xs font-medium text-black focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <AuthRedirectLink link={NAV_LINKS.sign_in} text={CONTENT.auth.to_signUp.text} boldText={CONTENT.auth.to_signUp.boldtext} />
        </div>
    );
}